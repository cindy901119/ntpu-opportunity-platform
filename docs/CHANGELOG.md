...
- 對外名稱更新為「鳶來有獎｜專屬北大人的 AI 獎助補助導航系統」。
- Navbar 與短版介面名稱使用「鳶來有獎」。
- 更新 metadata、首頁、提醒測試信與專案文件中的對外名稱。
- 收藏頁改為使用與機會頁相同的推薦演算法與目前偏好，避免收藏後推薦標籤全部變成「可以考慮」。
...
- 將主題切換改為站名旁的太陽／月亮開關，提升淺色與深色模式辨識度。
