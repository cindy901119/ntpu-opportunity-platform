...
function sourceItemKey(announcement: Announcement): string {
  return `${announcement.sourceKey}:${announcement.url}`;
}

function toInsertSql(announcements: Announcement[]): string {
...
async function main() {
  ...
  const announcements = JSON.parse(await readFile(inputPath, "utf8")) as Announcement[];
