...
export function RecommendationLabelTag({ label }: { label: RecommendationLabel }) {
  const className = {
    ...
    時間偏緊: "bg-[var(--warn-bg)] text-[var(--warn-text)]",
    已截止: "bg-[var(--paper-2)] text-[var(--attention)]",
    需要再確認: "bg-[var(--check-bg)] text-[var(--check-text)]",
  }[label];

  return <span className={`rounded-full px-2.5 py-1.5 text-xs font-semibold ${className}`}>{label}</span>;
}
